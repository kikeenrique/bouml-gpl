...
static QByteArray dot(".");

QByteArray UmlClass::java_stereotype()
{
  QByteArray s = JavaSettings::classStereotype(stereotype());
...
void UmlClass::generate(QTextStream & f, QByteArray indent) {
  ...
  const QByteArray & stereotype = java_stereotype();
...
void UmlClass::write(QTextStream & f) {
  if (isJavaExternal()) {
    QByteArray s = javaDecl().stripWhiteSpace();
    int index;
...
void UmlClass::import(QTextStream & f, const QByteArray & indent) {
  QByteArray s;
...
void UmlClass::generate_enum_pattern_case(QTextStream &, QByteArray) {
  // error already signaled
}

void UmlClass::generate_enum_member(QTextStream & f, QByteArray indent) {
  generate(f, indent + "  ");
  f << '\n';
}
...
void UmlClass::generate_import(QTextStream & f, const QByteArray & indent) {
  QVector<UmlItem> ch = children();
  const unsigned sup = ch.size();
  unsigned index;
