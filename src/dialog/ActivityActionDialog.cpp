...
void WithBehaviorDialog::init(BrowserNode * beh) {
  if (!visit) {
    ...
    QList<BrowserNode *>::const_iterator iter_node;

    QStringList::Iterator iter_str = node_names->begin();

    for (iter_node = (*nodes).begin(); iter_node!= (*nodes).end(); iter_node++, iter_str++) {
      behavior_co->insertItem(*((*iter_node)->pixmap(0)), *iter_str);
    }
