...
HideShowDialog::HideShowDialog(const BrowserNodeList & a,
  ...
  for (it = all.begin(); it!= all.end(); it++) {
    QString def = (*it)->get_data()->definition(TRUE, FALSE);

    (((hidden_visible.findIndex((*it)) == -1) ^ on_visible) ? lb_visible : lb_hidden) ->insertItem(new ListBoxBrowserNode((*it), def));
  }
...
void HideShowDialog::hide_private() {
  ...
  for (it = all.begin(); it!= all.end(); it++) {
    BasicData * m = (*it)->get_data();
    QString def = m->definition(TRUE, FALSE);

    ((((ClassMemberData *) m)->get_visibility(m->get_browser_node()) != UmlPrivate) ? lb_visible : lb_hidden)->insertItem(new ListBoxBrowserNode((*it), def));
  }
...
void HideShowDialog::hide_private_protected() {
  ...
  for (it = all.begin(); it!= all.end(); it++) {
    BasicData * m = (*it)->get_data();
    QString def = m->definition(TRUE, FALSE);
    UmlVisibility visi = ((ClassMemberData *) m)->get_visibility(m->get_browser_node());

    (((visi == UmlPublic) || (visi == UmlPackageVisibility)) ? lb_visible : lb_hidden)->insertItem(new ListBoxBrowserNode((*it), def));
  }
