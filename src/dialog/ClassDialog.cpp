...
ClassDialog::ClassDialog(ClassData * c) : Q3TabDialog(0, 0, TRUE), cl(c) {
  ...
  (new QLabel(TR("\nEnter formals in case the class is parameterized\n"),
              parametrized_vtab))->setAlignment(Qt::AlignHCenter | Qt::AlignTop);

  formals_table = new FormalParamsTable(c, parametrized_vtab, node_names, visit);
  ...
  if (cl->get_n_actualparams() != 0) {
    ...
    (new QLabel(TR("\nSpecify actuals else formals default value will be used\n"),
                instantiate_vtab))->setAlignment(Qt::AlignHCenter | Qt::AlignTop);

    actuals_table = new ActualParamsTable(c, instantiate_vtab, node_names, visit);
  ...
  if (bn->nestedp()) {
    bgv = cpp_visibility.init(htab, cl->get_cpp_visibility(), FALSE, 0, TR("follow uml"));
  ...
  editjavaannotation = new QPushButton((visit) ? TR("Show annotation") : TR("Edit annotation"),htab);
  ...
  if (!visit) {
    ...
    for (it = nodes.begin(); it!= nodes.end(); it++) {
      QString st = idl_stereotype(((ClassData *) ((*it)->get_data()))->get_stereotype());

      if (st == "enum") {
        edswitch_type->insertItem((*it)->full_name(TRUE));
      }
    }
  }
  ...
  if ((gp->get_type() == UmlPackage) &&
    ...
    applicableon_table = new ApplicableOnTable(grid, bn->get_value("stereotypeApplyOn"), visit);
...
static void cpp_generate_inherit(QString & s, ClassData * cl,
  ...
  for (it = inh.begin(); it!= inh.end(); it++) {
    RelationData * r = (RelationData *) (*it)->get_data();
...
static void java_generate_extends(QString & s, const QString & stereotype,
  ...
  for (it = inh.begin(); it!= inh.end(); it++) {
    RelationData * r = (RelationData *) ((BrowserRelation *) (*it))->get_data();

    if (r->get_javadecl_a()[0]) {
      bool gen = FALSE;
      BrowserClass * mother = r->get_end_class();
      QString other_stereotype = ClassDialog::java_stereotype(((ClassData *) mother->get_data())->get_stereotype());
...
static void php_generate_extends(QString & s, const QString & stereotype,
  ...
  for (it = inh.begin(); it!= inh.end(); it++) {
    ...
    if (r->get_phpdecl_a()[0]) {
      bool gen = FALSE;
      BrowserClass * mother = r->get_end_class();
      QString other_stereotype = ClassDialog::php_stereotype(((ClassData *) mother->get_data())->get_stereotype());
...
static void idl_generate_inherit(QString & s, QString st, ClassData * cl) {
  ...
  for (it = inh.begin(); it!= inh.end(); it++) {
    ...
    if (r->get_idldecl_a()[0]) {
      BrowserClass * mother = r->get_end_class();
      QString other_st = ClassDialog::idl_stereotype(mother->get_stereotype());
...
void ClassDialog::accept() {
  ...
  if (s != oldname) {
	...
	(!(err = ProfiledStereotypes::canAddStereotype(bn, s)).isEmpty())) {
      ...
      QString path = fromUnicode(ediconpath->text().simplifyWhiteSpace());
  ...
  if (stereotypetab != 0) {
    if (st == "stereotype") {
      ...
      QString oldiconpath = bn->get_value("stereotypeIconPath");
      QString newiconpath = fromUnicode(ediconpath->text().simplifyWhiteSpace());
...
void FormalParamsTable::update(ClassData * cl, BrowserNodeList & nodes) {
  ...
  for (index = 0; index != n; index += 1) {
    ...
    if (rank != -1)
      cl->set_formalparam_default_value(index, ((BrowserClass *) nodes.at(rank)),NULL);
    else
      cl->set_formalparam_default_value(index, 0, text(index, 2).stripWhiteSpace());

    rank = types.findIndex(text(index, 3).stripWhiteSpace());
    if (rank != -1)
      cl->set_formalparam_extends(index, ((BrowserClass *) nodes.at(rank)),NULL);
    else
...
void ActualParamsTable::generate(QString & s, ClassData * cl,
				 ...
				 BrowserNodeList & nodes,
				 QStringList & node_names) {
  ActualParamData * actual = 0;
  unsigned index;

  QList<ActualParamData *>::const_iterator it;

  for (it = (cl->actuals).begin(), index = 0; it!= (cl->actuals).end(); it++, index += 1) {
    actual = (*it);
