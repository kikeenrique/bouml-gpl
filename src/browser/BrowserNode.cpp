...
bool browserNodeLessThan(BrowserNode* s1, BrowserNode* s2)
{
  return s1 < s2;
}

bool shouldRemove(BrowserNode* first, BrowserNode* second)
{
...
bool BrowserNode::delete_internal(QString & warning) {
  ...
  if (!root_permission()) {
    ...
    for (it = targetof.begin(); it!= targetof.end(); it++) {
      BrowserNode * r = (*it);
...
QString BrowserNode::check_inherit(const BrowserNode * new_parent) const {
  ...
  if (notyet.indexOf(const_cast<BrowserNode *>(new_parent)) != -1) {
  ...
  do {
    ...
    if (all_parents.indexOf(cl) == -1) {
      all_parents.append(cl);

      QList<BrowserNode *> grand_parents = cl->parents();
      QList<BrowserNode *>::const_iterator it;

      for (it = grand_parents.begin(); it!= grand_parents.end(); it++) {
        cl = (*it);
        if (notyet.indexOf(cl) == -1) {
          notyet.append(cl);
        }
      }
    }
...
void BrowserNode::mark_management(int choice) {
  ...
  switch (choice) {
    ...
    {
      // move into
      QList<BrowserNode *>::const_iterator it = marked_list.end();
      while (it != marked_list.begin()) {
        --it;
        bn = (*it);
        move(bn, 0);
      }
  ...
  case referencedByCmd:
    {
      ...
      QList<BrowserNode *>::iterator it = std::unique (targetof.begin(), targetof.end(), shouldRemove);
      targetof.erase(it, targetof.end());
//       targetof.first();
...
void BrowserNode::signal_unconsistencies()
{
  ...
  if (!UnconsistencyFixedMsg.isEmpty()) {
    ...
    do {
      ModifiedPackages.takeAt(0)->is_modified = TRUE;
...
void BrowserNode::unconsistent_fixed(const char * what, BrowserNode * newone) {
  ...
  if (ModifiedPackages.indexOf(bn) == -1)
...
//   sort();
  qSort(this->begin(), this->end(), browserNodeLessThan);
...
void BrowserNodeList::full_defs(QStringList & list) const {
  ...
  QList<BrowserNode *>::const_iterator it;
  for (it = this->begin(); it!= this->end(); it++) {
    list.append((*it)->get_data()->definition(TRUE, FALSE));
  }
}
