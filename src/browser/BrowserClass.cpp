...
void BrowserClass::menu() {
  QList<BrowserOperation *> lraw = inherited_operations(21);
  Q3PopupMenu m(0);
  ...
  if (!deletedp()) {
	...
	if (strcmp(stereotype, "typedef") && strcmp(stereotype, "enum_pattern")) {
	  ...
	  if ((lraw.count() != 0) && strcmp(stereotype, "union")) {
	    if (lraw.count() > 20)
	      m.setWhatsThis(m.insertItem(TR("Add inherited operation"), 9999),
			     TR("to redefine an inherited <i>operation</i> in the <i>class</i>"));
	    else {
          QList<BrowserOperation *>::const_iterator l_iterator;

	      inhopersubm.insertItem(new MenuTitle(TR("Choose operation to add it"), m.font()), -1);
	      inhopersubm.insertSeparator();

	      for (l_iterator = lraw.begin(), index = 10000;
		   l_iterator != lraw.end();
		   l_iterator++, index += 1) {
		if (((OperationData *) (*l_iterator)->get_data())->get_is_abstract())
		  inhopersubm.insertItem(new MenuItalic(((BrowserNode *) (*l_iterator)->parent())->get_name() +
							QString("::") + (*l_iterator)->get_data()->definition(TRUE, FALSE),
							inhopersubm.font()),
					 index);
		else
		  inhopersubm.insertItem(((BrowserNode *) (*l_iterator)->parent())->get_name() +
					 QString("::") + (*l_iterator)->get_data()->definition(TRUE, FALSE),
					 index);
	      }
...
<i>relations</i> (except if the class at the other side is also deleted)"));
  ...
  exec_menu_choice(m.exec(QCursor::pos()), lraw);
}
...
void BrowserClass::get_all_parents(QList<BrowserClass *> & l) const {
  ...
  while (! notyet.isEmpty()) {
    BrowserClass * cl = (BrowserClass *) notyet.takeFirst();

    if ((cl != this) && (l.indexOf(cl) == -1)) {
      l.append(cl);

      QList<BrowserNode *> grand_parents = cl->parents();
      QList<BrowserNode *>::const_iterator l_iterator;
      for (l_iterator = grand_parents.begin();
           l_iterator != grand_parents.end();
           l_iterator++) {
        BrowserNode * gp = *l_iterator;
        if (notyet.indexOf(gp) == -1){
                notyet.append(gp);
        }
      }
    }
...
void BrowserClass::get_attrs(BrowserNodeList & attributes) const {
  ...
  get_all_parents(lcl);
  lcl.append(const_cast<BrowserClass *>(this));
  QList<BrowserClass *>::const_iterator it;

  for (it = lcl.begin(); it!= lcl.end(); it++) {
    Q3ListViewItem * child;

    for (child = (*it)->firstChild(); child; child = child->nextSibling())
      if ( !((BrowserNode *) child)->deletedp() &&
...
void BrowserClass::get_opers(Q3ValueList<const OperationData *> & opers,
  ...
  for (it = all_parents.begin(); it!= all_parents.end(); it++) {
  ...
  for (it = from.begin(); it!= from.end(); it++) {
    ...
    for (child = (*it)->firstChild(); child; child = child->nextSibling()) {
      if (!((BrowserNode *) child)->deletedp()) {
        ...
        case UmlDirectionalAggregationByValue:
          {
            RelationData * rd = (RelationData *) ((BrowserRelation *) child)->get_data();
            if ( (rels.indexOf(rd) == -1) &&
                 (to.indexOf(rd->get_end_class()) != -1) ) {
              rels.append(rd);
            }
          }
        default:
...
bool BrowserClass::may_contains_them(const QList<BrowserNode *> & l,
  ...
  for (it = l.begin(); it!= l.end(); it++) {
    switch ((*it)->get_type()) {
    case UmlOperation:
      if (((BrowserOperation *) (*it))->get_get_of())
        if (l.count(((BrowserOperation *) (*it))->get_get_of()) == 0)
          return FALSE;
      if (((BrowserOperation *) (*it))->get_set_of())
        if (l.count(((BrowserOperation *) (*it))->get_set_of()) == 0)
          return FALSE;
      break;
    case UmlAttribute:
      if (((BrowserAttribute *) (*it))->get_get_oper())
        if (l.count(((BrowserAttribute *) (*it))->get_get_oper()) == 0)
          return FALSE;
      if (((BrowserAttribute *) (*it))->get_set_oper())
        if (l.count(((BrowserAttribute *) (*it))->get_set_oper()) == 0)
          return FALSE;
      break;
    case UmlClass:
    case UmlExtraMember:
      break;
    case UmlDependOn:
      if (((const BrowserNode *) (*it)->parent()) != this)
        return FALSE;
      duplicable = FALSE;
      return TRUE;
    default:
      if (!IsaRelation((*it)->get_type()) ||
          (((const BrowserNode *) (*it)->parent()) != this))
        return FALSE;
      duplicable = FALSE;
      return TRUE;
    }
  ...
  for (it = all_parents.begin(); it!= all_parents.end(); it++) {
    ...
    for (child = cl->firstChild(); child; child = child->nextSibling()) {
      if ((((BrowserNode *) child)->get_type() == UmlOperation) &&
          ...
          (((BrowserNode *) child)->get_name() != cl->name) &&		// not a constructor
          (((BrowserNode *) child)->get_name()[0] != '~')) {// not a destructor
        OperationData * data = (OperationData *) ((BrowserOperation *) child)->get_data();
          ...
          if (other == 0) {
            ...
            if (l.count() >= (int)limit) {
              ...
              return l;
                    ...
                    ((OperationData *) other->get_data())->get_is_abstract()) {
		   ...
		   QList<BrowserClass *> & metaclasses)
{
  ...
  if (index != -1) {
    ...
    if (dflt) {
      for (it = metaclasses.begin(); it!= metaclasses.end(); it++) {
        BrowserClass * mcl = (*it);
    ...
    if (mcl == 0) {
      mcl = add_metaclass(cl, mclname, (dflt) ? Q3CString() : path);
