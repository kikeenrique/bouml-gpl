...
void BrowserUseCaseDiagram::menu() {
  ...
  if (!deletedp()) {
	...
	m.setWhatsThis(m.insertItem("Edit drawing settings", 2),
		       "to set how the <em>use case diagram</em>'s items must be drawn");
	m.insertSeparator();
...
bool BrowserUseCaseDiagram::get_show_stereotype_properties(UmlCode) const {
  ...
  case UmlNo:
    return FALSE;
  default:
    return ((BrowserNode *) parent())->get_show_stereotype_properties(UmlUseCaseDiagram);
  }
}
