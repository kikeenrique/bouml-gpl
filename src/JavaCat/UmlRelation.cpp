			  ...
			  )
{
  ...
  if (!roundtrip ||
    ...
    if (rel == 0) {
      JavaCatWindow::trace(QByteArray("<font face=helvetica><b>cannot add relation <i>")
...
# endif
  ...
  QByteArray decl = JavaSettings::relationDecl(array);
			...
			)
{
  ...
  QByteArray st = JavaSettings::umlType(genericname);
