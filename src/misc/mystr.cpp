...
QStringList SharedStr::shared[SHAREDDICTSIZE];
...
char * SharedStr::assign(const char * s, int len) {
  QStringList l = shared[len & (SHAREDDICTSIZE-1)];
  int pos = l.indexOf(s);
  if (pos == -1) {
    l.append(s);
  }
  int posfin = l.indexOf(s);

  return p = l.value(posfin).toLatin1().data();
}
...
MayBeSharedStr::MayBeSharedStr(const MayBeSharedStr & s) : SharedStr() {
  if ((s.p[0] == 0) ||
      (shared[strlen(s.p) & (SHAREDDICTSIZE-1)].indexOf(s.p) != -1)) {
    // shared
    p = s.p;
  } else {
...
MayBeSharedStr::~MayBeSharedStr() {
  ...
  if (*p && (shared[strlen(p) & (SHAREDDICTSIZE-1)].indexOf(p) == -1)) {
    // not shared
    delete [] p;
  }
}
