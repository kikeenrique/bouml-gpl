...
QByteArray UmlBasePackage::phpNamespace() {
  read_if_needed_();
  
  return _php_namespace;
}

bool UmlBasePackage::set_PhpNamespace(const QByteArray & s) {
  return set_it_(_php_namespace, s, setPhpNamespaceCmd);
}

UmlPackage * UmlBasePackage::findPhpNamespace(const QByteArray & n) const {
