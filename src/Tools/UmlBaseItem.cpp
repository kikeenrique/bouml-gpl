...
QPtrDict<UmlItem> UmlBaseItem::_all(203);

UmlBaseItem::UmlBaseItem(void * id, const QCString & n)
    : _defined(FALSE), _marked(FALSE), _identifier(id), _name(n), _parent(0), _children(0) {
  _all.insert(id, (UmlItem *) this);
  if ((_all.count() / 2) >= _all.size())
...
// until they are defined for all the types
...
UmlItem * UmlBaseItem::read_()
{
  ...
  if (result == 0) {
    ...
    case aPackage:
      ...
      break;
    case aState:
      result = new UmlState(id, name);
      break;
    case aTransition:
      result = new UmlTransition(id, name);
      break;
    case aRegion:
      result = new UmlRegion(id, name);
      break;
    case aStateDiagram:
      result = new UmlStateDiagram(id, name);
      break;
    case aStateAction:
      result = new UmlStateAction(id);
      break;
    case anInitialPseudoState:
      result = new UmlInitialPseudoState(id);
      break;
    case anEntryPointPseudoState:
      result = new UmlEntryPointPseudoState(id, name);
      break;
    case aFinalState:
      result = new UmlFinalState(id);
      break;
    case aTerminatePseudoState:
      result = new UmlTerminatePseudoState(id);
      break;
    case anExitPointPseudoState:
      result = new UmlExitPointPseudoState(id, name);
      break;
    case aDeepHistoryPseudoState:
      result = new UmlDeepHistoryPseudoState(id);
      break;
    case aShallowHistoryPseudoState:
      result = new UmlShallowHistoryPseudoState(id);
      break;
    case aJunctionPseudoState:
      result = new UmlJunctionPseudoState(id);
      break;
    case aChoicePseudoState:
      result = new UmlChoicePseudoState(id);
      break;
    case aForkPseudoState:
      result = new UmlForkPseudoState(id);
      break;
    case aJoinPseudoState:
      result = new UmlJoinPseudoState(id);
      break;
    default:
      UmlCom::bye();
...
bool UmlBaseItem::set_it_(UmlTypeSpec & r, const UmlTypeSpec & t,
			  OnInstanceCmd cmd) {
...
bool UmlBaseItem::moveAfter(UmlItem * x) {
