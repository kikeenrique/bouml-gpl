...
bool ComponentData::tool_cmd(ToolCom * com, const char * args,
  ...
  if (((unsigned char) args[-1]) >= firstSetCmd) {
	...
	if (associated != 0) {
	  ...
	  while ((*it)) {
	    disconnect((*it)->get_data(), SIGNAL(deleted()),
		       this, SLOT(on_delete()));
	    ++it;
	  }
...
void ComponentData::on_delete() {
  ...
  if (associated != 0) {
    ...
    while ((*it)) {
      if ((*it)->deletedp()) {
	modp = TRUE;
	associated->remove((*it)); // update it
      }
      else
	++it;
    }
...
void ComponentData::update_associated(QPtrDict<BrowserComponent> & d) {
  ...
  while ((*it)) {
    if (associated->find((*it)) == 0) {
      associated->insert((*it), (*it));
      if (associated->size() < (associated->count() >> 4))
	associated->resize((associated->count() >> 4) + 1);
      connect((*it)->get_data(), SIGNAL(deleted()),
	      this, SLOT(on_delete()));
    }
    ++it;
  }
...
void ComponentData::save(QTextStream & st, QString & warning) const {
  ...
  if (associated != 0) {
	...
	warning += QString("<p><b>") + browser_node->get_name() + 
	  "</b>'s associated component <b>" + (*it)->get_name() +
	    "</b> is deleted\n";
      else {
	nl_indent(st);
