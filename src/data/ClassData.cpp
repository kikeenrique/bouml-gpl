...
ClassData::ClassData(const ClassData * model, BrowserNode * bn)
  ...
  for (it = (model->actuals).begin(); it!= (model->actuals).end(); it++) {
    ActualParamData * actual = new ActualParamData(*((*it)));
...
void ClassData::update_actuals(BrowserClass * parent,
  ...
  if (n != 0) {
    ...
    for (it = actuals.begin(); it!= actuals.end(); it++) {
      ActualParamData * actual = (*it);
      if ((actual->get_class() == parent) &&
          (managed.indexOf(actual) == -1)){
        // find;
        break;
      }
    }
    ...
    while (actual && (nth < n) && (actual->get_class() == parent)) {
      ...
      //EGA this use of iterators may break real intentions
//       actual = actuals.next();
      actual = *(++it);
      nth += 1;
    }
...
void ClassData::update_actuals() {
  ...
  for (it = parents.begin(); it!= parents.end(); it++) {
    BrowserClass * parent = (BrowserClass *) (*it);
  ...
  if (!(actuals == new_actuals)) {
    ...
    for (it = actuals.begin(); it!= actuals.end(); it++) {
      ActualParamData * actual = (*it);
      if (new_actuals.indexOf(actual) == -1) {
        delete actual;
      }
    }
...
QString ClassData::get_actuals(BrowserClass * parent) {
  ...
  QList<ActualParamData *>::const_iterator it2;
...
bool ClassData::tool_cmd(ToolCom * com, const char * args,
  ...
  if (((unsigned char) args[-1]) >= firstSetCmd) {
	...
	{
	  int rank = com->get_unsigned(args);
