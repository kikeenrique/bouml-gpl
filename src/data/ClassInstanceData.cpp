...
void ClassInstanceData::init_other_side() {
  ...
  while (it_rel != relations.end()) {
	...
	if (other_data->connect_list.indexOf(d) == -1) {
...
void ClassInstanceData::check() {
  if ((cl == 0) || // deleted class instance referenced in a diagram
    ...
    while (it_attr != attributes.end()) {
	  ...
	  (l.indexOf((BrowserClass *) slot_attr.att->parent())  == -1)) {
	// must be removed
	...
	modif = TRUE;
      ...
      else if (l.indexOf((BrowserClass *) br->parent()) == -1) {
...
void ClassInstanceData::add(BrowserClassInstance * other,
  ...
  if (!RelationData::uni_directional(rd->get_type())) {
    ...
    if (od->connect_list.indexOf(d) == -1) {
...
void ClassInstanceData::set_class(BrowserClass * t) {
  if (t != cl) {
    ...
    while (!connect_list.isEmpty())
      disconnect(connect_list.takeAt(0), 0, this, 0);
  ...
  if (((unsigned char) args[-1]) >= firstSetCmd) {
      ...
      case setAttributeCmd:
	{
	  ...
	  if (! find) {
	    ...
	    if (at->deletedp() ||
		(l.indexOf((BrowserClass *) at->parent())  == -1)) {
	      // illegal
	      com->write_ack(FALSE);
	      return TRUE;
	    }
    ...
    case relationsCmd:
      {
        ...
        if (other == 0) {
          ...
          QList<BrowserRelation *>::const_iterator it;

          for (it = l.begin(); it!= l.end(); it++) {
            BrowserRelation * r = (*it);
            r->write_id(com);
          }
        }
      }
