    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((SdMsgCanvas *) (*it))->drawing_language = drawing_language;
        }
        if (!st[1].name.isEmpty()) {
          ((SdMsgCanvas *) (*it))->show_full_oper = show_full_oper;
        }
        if (!st[2].name.isEmpty()) {
          ((SdMsgCanvas *) (*it))->show_context_mode = show_context_mode;
        }
        ((SdMsgCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void SdMsgCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  SdMsgCanvas * x = (SdMsgCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    SdMsgCanvas * o =  (SdMsgCanvas *) (*it);
