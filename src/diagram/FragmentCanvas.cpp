...
void FragmentCanvas::moveBy(double dx, double dy) {
  ...
  QList<FragmentSeparatorCanvas *>::const_iterator it;
  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((FragmentCanvas *) (*it))->itscolor = itscolor;
        ((FragmentCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void FragmentCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  FragmentCanvas * x = (FragmentCanvas *) (*it);
  it++;
  for ( ;it!= l.end(); it++) {
    FragmentCanvas * o =  (FragmentCanvas *) (*it);
			  ...
			  QList<FragmentCanvas *> & refs)
{
  ...
  for (it = fragments.begin(); it!= fragments.end(); it++) {
    ...
    if (sz == 0)
      ...
      unsigned index;
      QList<FragmentSeparatorCanvas *>::const_iterator it2;
      for (index = 0, it2 = (f->separators).begin(); it2!= (f->separators).end(); it2++, index += 1) {
        v[index] = (*it2);
      }
