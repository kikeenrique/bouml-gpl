  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((SubjectCanvas *) (*it))->itscolor = itscolor;
        ((SubjectCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void SubjectCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  SubjectCanvas * x = (SubjectCanvas *) (*it);

  it++;
  for (; it!= l.end(); it++) {
    SubjectCanvas * o =  (SubjectCanvas *) (*it);
...
void SubjectCanvas::send(ToolCom * com, Q3CanvasItemList & all)
{
  ...
  QList<SubjectCanvas *>::const_iterator it;

  for (it = subjects.begin(); it!= subjects.end(); it++) {
    SubjectCanvas * sc = (*it);
    QString s = fromUnicode(sc->name);
