  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ParameterSetCanvas *) (*it))->itscolor = itscolor;
        ((ParameterSetCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ParameterSetCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ParameterSetCanvas * x = (ParameterSetCanvas *) (*it);

  it++;
  for (; it!= l.end(); it++) {
    ParameterSetCanvas * o =  (ParameterSetCanvas *) (*it);
