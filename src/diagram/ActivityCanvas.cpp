    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((ActivityCanvas *) (*it))->settings.show_infonote = show_infonote;
        }
        if (!st[1].name.isEmpty()) {
          ((ActivityCanvas *) (*it))->settings.drawing_language = drawing_language;
        }
        if (!co[0].name.isEmpty()) {
          ((ActivityCanvas *) (*it))->itscolor = itscolor;
        }
        ((ActivityCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ActivityCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ActivityCanvas * x = (ActivityCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ActivityCanvas * o =  (ActivityCanvas *) (*it);
