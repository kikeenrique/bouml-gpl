    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((SdSelfMsgCanvas *) (*it))->drawing_language =  drawing_language;
        }
        if (!st[1].name.isEmpty()) {
          ((SdSelfMsgCanvas *) (*it))->show_full_oper = show_full_oper;
        }
        if (!st[2].name.isEmpty()) {
          ((SdSelfMsgCanvas *) (*it))->show_context_mode =  show_context_mode;
        }
        ((SdSelfMsgCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void SdSelfMsgCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  SdSelfMsgCanvas * x = (SdSelfMsgCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    SdSelfMsgCanvas * o =  (SdSelfMsgCanvas *) (*it);
