...
void ArtifactCanvas::draw_all_relations() {
  if (strcmp(browser_node->get_stereotype(), "source") != 0) {
    ...
    for (it = lines.begin(); it!= lines.end(); it++) {
      if (((*it)->type() == UmlContain) &&
          (((AssocContainCanvas *) (*it))->get_start() == this)) {
        (*it)->delete_it();
      }
    }
...
void ArtifactCanvas::update_relations(ArtifactCanvas * other) {
  ...
  const Q3PtrDict<BrowserArtifact> * associated =  ((ArtifactData *) browser_node->get_data())->get_associated();
  bool association_must_exist = ((associated != 0) &&
                                 (associated->find((BrowserArtifact *) other->browser_node) != 0));
  QList<ArrowCanvas *>::iterator it;

  for (it = lines.begin(); it!= lines.end(); it++) {
    if (((*it)->type() == UmlContain) &&
        (((AssocContainCanvas *) (*it))->get_end() == other)) {
...
void ArtifactCanvas::update_relations() {
  ...
  for (it = lines.begin(); it!= lines.end(); it++) {
    if (((*it)->type() == UmlContain) &&
        (((AssocContainCanvas *) (*it))->get_start() == this)) {
      DiagramItem * adi = ((AssocContainCanvas *) (*it))->get_end();

      if ((adi->type() == UmlArtifact) &&
          (associated != 0) &&
          (associated->find((BrowserArtifact *) ((ArtifactCanvas *) adi)->browser_node) != 0)) {
  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ArtifactCanvas *) (*it))->itscolor = itscolor;
        ((ArtifactCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ArtifactCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ArtifactCanvas * x = (ArtifactCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ArtifactCanvas * o =  (ArtifactCanvas *) (*it);
