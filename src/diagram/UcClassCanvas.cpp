    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!co[0].name.isEmpty()) {
          ((UcClassCanvas *) (*it))->itscolor = itscolor;
        }
        ((UcClassCanvas *) (*it))->settings.set(st, 0);
        ((UcClassCanvas *) (*it))->modified();
        ((UcClassCanvas *) (*it))->package_modified();
      }
    }
    if (!dialog.redo())
      break;
...
void UcClassCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  UcClassCanvas * x = (UcClassCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    UcClassCanvas * o =  (UcClassCanvas *) (*it);
