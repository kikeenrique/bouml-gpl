  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((UcUseCaseCanvas *) (*it))->itscolor = itscolor;
        ((UcUseCaseCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void UcUseCaseCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  UcUseCaseCanvas * x = (UcUseCaseCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    UcUseCaseCanvas * o =  (UcUseCaseCanvas *) (*it);
...
void UcUseCaseCanvas::send(ToolCom * com, Q3CanvasItemList & all)
{
  ...
  QList<UcUseCaseCanvas *>::const_iterator itu;
  for (itu = lu.begin(); itu!= lu.end(); itu++) {
    com->write_unsigned((unsigned) (*itu)->get_ident());
    (*itu)->get_bn()->write_id(com);
    com->write((*itu)->rect());
  }
  ...
  QList<UcClassCanvas *>::const_iterator ita;
  for (ita = la.begin(); ita!= la.end(); ita++) {
    (*ita)->get_bn()->write_id(com);
  }
  ...
  for (itu = lu.begin(); itu!= lu.end(); itu++) {
    QList<ArrowCanvas *>::const_iterator itl;

    for (itl = ((*itu)->lines).begin(); itl!= ((*itu)->lines).end(); itl++) {
      ArrowCanvas * r = (*itl);
  ...
  QList<ArrowCanvas *>::const_iterator itr;

  for (itr = lr.begin(); itr!= lr.end(); itr++) {
    (*itr)->write_uc_rel(com);
  }
}
