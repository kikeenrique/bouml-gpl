...
void SdDurationCanvas::update_hpos() {
  ...
  QList<SdDurationCanvas *>::const_iterator it;
...
void SdDurationCanvas::moveBy(double dx, double dy) {
  if (dy > 80000) {
    ...
    QList<SdMsgBaseCanvas *>::const_iterator it;
...
void SdDurationCanvas::cut_internal(int py) {
  QList<SdDurationCanvas *>::const_iterator itd;

  for (itd = durations.begin(); itd!= durations.end(); itd++) {
    (*itd)->cut_internal(py);
  }
  ...
  for (it = ms.begin(); it!= ms.end(); it++) {
    if ((*it)->rect().center().y() > py) {
      ...
      remove((*it));
      newone->add((*it));
      (*it)->change_duration(this, newone);
    }
  }

  QList<SdDurationCanvas *> durs(durations);
  QList<SdDurationCanvas *>::const_iterator itdur;

  for (itd = durs.begin(); itd!= durs.end(); itd++) {
    if ((*itdur)->rect().center().y() > py) {
      // on the new duration
      if (newone == 0) {
        // width and height to 0 on creation to not have
        // a round error because of the zoom
      ...
      remove((*itdur));
      newone->add((*itdur));
      (*itdur)->support =  newone;
    }

    (*itdur)->update_v_to_contain((*itdur), TRUE);
  }
}
...
void SdDurationCanvas::collapse(SdDurationCanvas * d) {
  ...
  while (! durs.isEmpty()) {
    SdDurationCanvas * dur = durs.takeAt(0);
...
void SdDurationCanvas::postToOverlapping() {
  ...
  for (it = msgs.begin(); it!= msgs.end(); it++) {
    (*it)->upper();
    (*it)->update_hpos();
    ((DiagramItem *) (*it))->update();	// not QCanvasItem::update
  }
}
...
void SdDurationCanvas::edit_drawing_settings(QList<DiagramItem *> & l) {
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      QList<DiagramItem *>::const_iterator it;

      for (it = l.begin(); it!= l.end(); it++) {
        ((SdDurationCanvas *) (*it))->itscolor = itscolor;
        ((SdDurationCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void SdDurationCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  QList<DiagramItem *>::const_iterator it = l.begin();

  SdDurationCanvas * x = (SdDurationCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    SdDurationCanvas * o =  (SdDurationCanvas *) (*it);
...
void SdDurationCanvas::propag_visible(QList<SdDurationCanvas *> & l, bool y)
{
  ...
  for (itd = l.begin(); itd!= l.end(); itd++) {
    SdDurationCanvas * d = (*itd);
    d->Q3CanvasItem::setVisible(y);

    QList<SdMsgBaseCanvas *>::const_iterator itm;
    for (itm = (d->msgs).begin(); itm!= (d->msgs).end(); itm++) {
      (*itm)->Q3CanvasItem::setVisible(y);
    }
...
unsigned SdDurationCanvas::count_msg(int api_format) const {
  ...
  for (itm = msgs.begin(); itm!= msgs.end(); itm++) {
    switch ((*itm)->type()) {
    case UmlSyncMsg:
    case UmlAsyncMsg:
    case UmlReturnMsg:
      if ((*itm)->get_dest() == (const SdMsgSupport *) this) {
        break;
      }
      // no break
    default:
  ...
  for (itd = durations.begin(); itd!= durations.end(); itd++) {
    count += (*itd)->count_msg(api_format);
  }
