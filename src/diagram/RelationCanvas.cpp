...
void RelationCanvas::show_assoc_class(CdClassCanvas * cc) {
  ...
  while (rc->begin->type() == UmlArrowPoint) {
    ...
    rc = (RelationCanvas *) ((ArrowPointCanvas *) rc->begin)->get_other(rc);
  }
  ...
  for (;;) {
    ...
    if (rc->end->type() == UmlArrowPoint) {
      rc = (RelationCanvas *) ((ArrowPointCanvas *) rc->end)->get_other(rc);
    } else {
      break;
    }
  }
...
void RelationCanvas::hide_assoc_class() {
  ...
  for (;;) {
    ...
    for (it = (rc->lines).begin(); it!= (rc->lines).end(); it++) {
      ArrowCanvas * a = (*it);
