  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((InterruptibleActivityRegionCanvas *) (*it))->itscolor = itscolor;
        ((InterruptibleActivityRegionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void InterruptibleActivityRegionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  InterruptibleActivityRegionCanvas * x = (InterruptibleActivityRegionCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    InterruptibleActivityRegionCanvas * o =  (InterruptibleActivityRegionCanvas *) (*it);
