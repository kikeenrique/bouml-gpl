...
int ColMsgList::compareItems(ColMsg *item1, ColMsg *item2) {
  unsigned r1 = (item1)->get_rank();
  unsigned r2 = (item2)->get_rank();
...
void ColMsg::delete_it_internal() {
  // delete sub msgs
  while (!msgs.isEmpty()) {
    msgs.takeFirst()->delete_it_internal();
  }
...
bool ColMsg::extract_it(ColMsgList & top) {
  QList<ColMsg*>::const_iterator it;
  int index;
  for (index=0, it = top.begin(); it!= top.end(); index++, it++) {
    if ((*it) == this) {
      top.removeAt(index);
      return TRUE;
    }

    if (extract_it((*it)->msgs)) {
      return TRUE;
    }
...
void ColMsg::update_ranks(ColMsgList & msgs)
{
  ...
  for (hi_rank = 1, it = msgs.begin(); it!= msgs.end(); it++,hi_rank += 1) {
    (*it)->update_ranks(rank, QString::number(hi_rank));	// rank is updated
  }
}
...
void ColMsg::update_ranks(unsigned & r, QString hr) {
  ...
  for (hi_rank = 1, it = msgs.begin(); it!= msgs.end(); it++,hi_rank += 1) {
    (*it)->update_ranks(r, hr + QString::number(hi_rank));
  }
}
...
void ColMsg::place_in_its_support() {
  ...
  for (index = 0,it = (in->get_msgs()).begin(); it!= (in->get_msgs()).end(); it++,index += 1) {
    if (!gt(hr, (*it)->hierarchical_rank)) {
...
void ColMsg::place_in_internal(ColMsgList & l) {
  ColMsgList * pl = &l;
  bool ndots = hierarchical_rank.contains('.');

  for (;;) {
    unsigned index = pl->count();
    QList<ColMsg *>::const_iterator it = (*pl).end();
    ColMsg * m = (*it);
    while (it != (*pl).begin()) {
      m=(*it);
				...
				const ColMsgList & msgs)
{
  ...
  for (it = msgs.begin(); it!= msgs.end(); it++) {
    ColMsgList & l = ((*it)->is_forward) ? all_in : all_out;
    const QString & hi = (*it)->hierarchical_rank;

    unsigned index;
    QList<ColMsg *>::const_iterator it2;

    for (index = 0, it2 = l.begin(); it2!= l.end(); it2++, index += 1) {
      if (lt(hi, (*it2)->hierarchical_rank)) {
        break;
      }
    }
...
ColMsg * ColMsg::find_rec(const QString & hr, ColMsgList & top)
{
  ...
  for (index = 0, it = top.begin(); it!= top.end(); it++, index +=1) {
    if ((*it)->hierarchical_rank == hr) {
      return (*it);
    }

    ColMsg * result = find_rec(hr, (*it)->msgs);
  ...
  for (it = l.begin(); it!= l.end(); it++) {
    ...
    if (msg->operation != 0) {
      if (msg->operation->deletedp()) {
        BrowserOperation * br_op =  (BrowserOperation *) msg->operation->get_browser_node();
