    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((ActivityActionCanvas *) (*it))->show_opaque_action_definition = show_opaque_action_definition;
        }
        if (!co[0].name.isEmpty()) {
          ((ActivityActionCanvas *) (*it))->itscolor = itscolor;
        }
        ((ActivityActionCanvas *) (*it))->settings.set(st, 1);
        ((ActivityActionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ActivityActionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ActivityActionCanvas * x = (ActivityActionCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ActivityActionCanvas * o =  (ActivityActionCanvas *) (*it);
