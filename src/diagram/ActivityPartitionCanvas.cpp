  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ActivityPartitionCanvas *) (*it))->itscolor = itscolor;
        ((ActivityPartitionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ActivityPartitionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ActivityPartitionCanvas * x = (ActivityPartitionCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ActivityPartitionCanvas * o =  (ActivityPartitionCanvas *) (*it);
