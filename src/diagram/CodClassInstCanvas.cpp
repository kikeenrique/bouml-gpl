    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty())
          ((CodClassInstCanvas *) (*it))->write_horizontally =
            write_horizontally;
        if (!st[1].name.isEmpty())
          ((CodClassInstCanvas *) (*it))->show_context_mode =
            show_context_mode;
        if (!co[0].name.isEmpty())
          ((CodClassInstCanvas *) (*it))->itscolor = itscolor;
        ((CodClassInstCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void CodClassInstCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  CodClassInstCanvas * x = (CodClassInstCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    CodClassInstCanvas * o =  (CodClassInstCanvas *) (*it);
...
void CodClassInstCanvas::send(ToolCom * com, Q3CanvasItemList & all)
{
  ...
  QList<CodClassInstCanvas *>::const_iterator it;

  for (it = l.begin(); it!= l.end(); it++) {
    CodClassInstCanvas * i = (*it);
