    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty())
          ((TransitionCanvas *) (*it))->drawing_language =
            drawing_language;
        if (!st[1].name.isEmpty())
          ((TransitionCanvas *) (*it))->write_horizontally =
            write_horizontally;
        if (!st[2].name.isEmpty())
          ((TransitionCanvas *) (*it))->show_definition =
            show_definition;
        ((TransitionCanvas *) (*it))->propagate_drawing_settings();
        ((TransitionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void TransitionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  TransitionCanvas * x = (TransitionCanvas *) (*it);
  it++;
  for (it = l.begin(); it!= l.end(); it++) {
    TransitionCanvas * o =  (TransitionCanvas *) (*it);
