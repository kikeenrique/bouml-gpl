    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((DeploymentNodeCanvas *) (*it))->write_horizontally = write_horizontally;
        }
        if (!st[1].name.isEmpty()) {
          ((DeploymentNodeCanvas *) (*it))->show_stereotype_properties = show_stereotype_properties;
        }
        if (!co[0].name.isEmpty()) {
          ((DeploymentNodeCanvas *) (*it))->itscolor = itscolor;
        }
        ((DeploymentNodeCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void DeploymentNodeCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  DeploymentNodeCanvas * x = (DeploymentNodeCanvas *) (*it);

  for (; it!= l.end(); it++) {
    DeploymentNodeCanvas * o =  (DeploymentNodeCanvas *) (*it);
