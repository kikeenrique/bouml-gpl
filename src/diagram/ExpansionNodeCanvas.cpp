  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ExpansionNodeCanvas *) (*it))->itscolor = itscolor;
        ((ExpansionNodeCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ExpansionNodeCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ExpansionNodeCanvas * x = (ExpansionNodeCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ExpansionNodeCanvas * o =  (ExpansionNodeCanvas *) (*it);
