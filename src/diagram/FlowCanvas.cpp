    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((FlowCanvas *) (*it))->write_horizontally = write_horizontally;
        }
        ((FlowCanvas *) (*it))->settings.set(st, 1);
        ((FlowCanvas *) (*it))->propagate_drawing_settings();
        ((FlowCanvas *) (*it))->modified();
      }
    }
    if (!dialog.redo()) {
      break;
...
void FlowCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  FlowCanvas * x = (FlowCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    FlowCanvas * o =  (FlowCanvas *) (*it);
