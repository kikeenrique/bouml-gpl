			    ...
			    QList<FragmentCanvas *> & refs)
{
  ...
  for (cit = l.begin(); cit != l.end(); ++cit) {
    ...
    if ((it != 0) && // an uml canvas item
      ...
      QList<SdDurationCanvas *>::const_iterator iter;

      // standard msgs
      for (iter = (ll->durations).begin(); iter!= (ll->durations).end(); iter++) {
        n += (*iter)->count_msg(api_format);
      }
      ...
      if (com->api_format() >= 41) {
        ...
        QList<FragmentCanvas *>::const_iterator it;
  ...
  QList<SdLifeLineCanvas *>::const_iterator it;
  for (it = lls.begin(); it!= lls.end(); it++) {
    SdLifeLineCanvas * ll = (*it);
  ...
  if (com->api_format() >= 41) {
    ...
    QList<FragmentCanvas *>::const_iterator it2;

    for (it2 = fragments.begin(); it2!= fragments.end(); it2++) {
      FragmentCanvas * f = (*it2);
      QList<SdLifeLineCanvas *> covered;

      QList<SdLifeLineCanvas *>::const_iterator it3;

      for (it3 = lls.begin(); it3!= lls.end(); it3++) {
        ll = (*it3);
        if (f->collidesWith(ll)) {
          covered.append(ll);
        }
      ...
      QList<SdLifeLineCanvas *>::const_iterator it4;
      for (it4 = covered.begin(); it4!= covered.end(); it4++) {
        ll = (*it4);
