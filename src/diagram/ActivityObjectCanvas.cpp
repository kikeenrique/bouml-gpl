    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((ActivityObjectCanvas *) (*it))->write_horizontally = write_horizontally;
        }
        if (!co[0].name.isEmpty()) {
          ((ActivityObjectCanvas *) (*it))->itscolor = itscolor;
        }
        ((ActivityObjectCanvas *) (*it))->settings.set(st, 1);
        ((ActivityObjectCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ActivityObjectCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ActivityObjectCanvas * x = (ActivityObjectCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ActivityObjectCanvas * o =  (ActivityObjectCanvas *) (*it);
