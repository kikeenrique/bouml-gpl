...
void DiagramCanvas::moveSelfRelsBy(double dx, double dy) {
  ...
  for (it = lines.begin(); it!= lines.end(); it++) {
    if ((*it)->get_start() == (*it)->get_end()){
      // self relation
      (*it)->move_self_points(dx, dy); // move only if first segment => not done 2 times
    }
  }
}
...
bool DiagramCanvas::has_simple_relation(BasicData * def) const {
  QList<ArrowCanvas *>::const_iterator it;
  for (it = lines.begin(); it!= lines.end(); it++) {
    if (IsaSimpleRelation((*it)->type()) &&
        (((SimpleRelationCanvas *) (*it))->get_data() == def)) {
      return TRUE;
    }
  }
...
bool DiagramCanvas::has_transition(BasicData * def) const {
  QList<ArrowCanvas *>::const_iterator it;
  for (it = lines.begin(); it!= lines.end(); it++) {
    if (((*it)->type() == UmlTransition) &&
        (((TransitionCanvas *) (*it))->get_data() == def)) {
      return TRUE;
    }
  }
