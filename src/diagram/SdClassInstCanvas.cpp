    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty())
          ((SdClassInstCanvas *) (*it))->drawing_mode =
            drawing_mode;
        if (!st[1].name.isEmpty())
          ((SdClassInstCanvas *) (*it))->write_horizontally =
            write_horizontally;
        if (!st[2].name.isEmpty())
          ((SdClassInstCanvas *) (*it))->show_context_mode =
            show_context_mode;
        if (!co[0].name.isEmpty())
          ((SdClassInstCanvas *) (*it))->itscolor = itscolor;
        ((SdClassInstCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void SdClassInstCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  SdClassInstCanvas * x = (SdClassInstCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    SdClassInstCanvas * o =  (SdClassInstCanvas *) (*it);
...
void SdClassInstCanvas::send(ToolCom * com, Q3CanvasItemList & all)
{
  ...
  QList<SdClassInstCanvas *>::const_iterator it;

  for (it = l.begin(); it!= l.end(); it++) {
    SdClassInstCanvas * i = (*it);

    com->write_unsigned((unsigned) i->get_ident());
    if (i->browser_node->get_type() == UmlClass) {
      com->write_id(0);

      QString s = fromUnicode(i->iname);
