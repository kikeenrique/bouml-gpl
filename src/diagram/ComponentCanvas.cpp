...
void ComponentCanvas::modified() {
  ...
  QList<ArrowCanvas *>::iterator it = lines.begin();
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((ComponentCanvas *) (*it))->settings.draw_component_as_icon = draw_component_as_icon;
        }
        if (!st[1].name.isEmpty()) {
          ((ComponentCanvas *) (*it))->settings.show_component_req_prov = show_component_req_prov;
        }
        if (!st[2].name.isEmpty()) {
          ((ComponentCanvas *) (*it))->settings.show_component_rea = show_component_rea;
        }
        if (!st[3].name.isEmpty()) {
          ((ComponentCanvas *) (*it))->settings.show_stereotype_properties = show_stereotype_properties;
        }
        if (!co[0].name.isEmpty()) {
          ((ComponentCanvas *) (*it))->itscolor = itscolor;
        }
        ((ComponentCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ComponentCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ComponentCanvas * x = (ComponentCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ComponentCanvas * o =  (ComponentCanvas *) (*it);
