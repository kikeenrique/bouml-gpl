  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ConstraintCanvas *) (*it))->itscolor = itscolor;
        ((ConstraintCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ConstraintCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ConstraintCanvas * x = (ConstraintCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ConstraintCanvas * o =  (ConstraintCanvas *) (*it);
...
ConstraintCanvas * ConstraintCanvas::compute(UmlCanvas * canvas,
                                              CdClassCanvas * cl,
