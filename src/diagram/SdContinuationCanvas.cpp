  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((SdContinuationCanvas *) (*it))->itscolor = itscolor;
        ((SdContinuationCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void SdContinuationCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  SdContinuationCanvas * x = (SdContinuationCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    SdContinuationCanvas * o =  (SdContinuationCanvas *) (*it);
...
void SdContinuationCanvas::send(ToolCom * com, Q3CanvasItemList & all)
{
  ...
  for (cit = all.begin(); (cit != all.end()); ++cit) {
    ...
    if ((di != 0) && (*cit)->visible() && (di->type() == UmlContinuation)) {
      QString s = fromUnicode(((SdContinuationCanvas *) di)->name);
