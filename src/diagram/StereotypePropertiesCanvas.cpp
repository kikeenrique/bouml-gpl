  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((StereotypePropertiesCanvas *) (*it))->itscolor = itscolor;
        ((StereotypePropertiesCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void StereotypePropertiesCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  StereotypePropertiesCanvas * x = (StereotypePropertiesCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    StereotypePropertiesCanvas * o =  (StereotypePropertiesCanvas *) (*it);
