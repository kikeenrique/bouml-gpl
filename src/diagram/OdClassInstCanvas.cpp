...
bool OdClassInstCanvas::has_relation(const SlotRel & slot_rel) const {
  ...
  for (it = lines.begin(); it!= lines.end(); it++) {
    if (IsaRelation((*it)->type()) &&
        (((ObjectLinkCanvas *) (*it))->is(slot_rel, (*it)->get_start() == (DiagramItem *) this))) {
      return TRUE;
    }
  }
...
bool OdClassInstCanvas::is_duplicated(ObjectLinkCanvas * lnk,
  ...
  QList<ArrowCanvas *>::const_iterator it = lines.begin();
  ArrowCanvas * ar;
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((OdClassInstCanvas *) (*it))->write_horizontally =  write_horizontally;
        }
        if (!st[1].name.isEmpty()) {
          ((OdClassInstCanvas *) (*it))->show_context_mode = show_context_mode;
        }
        if (!co[0].name.isEmpty()) {
          ((OdClassInstCanvas *) (*it))->itscolor = itscolor;
        }
        ((OdClassInstCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void OdClassInstCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  OdClassInstCanvas * x = (OdClassInstCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    OdClassInstCanvas * o =  (OdClassInstCanvas *) (*it);
