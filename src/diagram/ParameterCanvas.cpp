  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ParameterCanvas *) (*it))->itscolor = itscolor;
        ((ParameterCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void ParameterCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ParameterCanvas * x = (ParameterCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ParameterCanvas * o =  (ParameterCanvas *) (*it);
