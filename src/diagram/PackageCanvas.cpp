...
void PackageCanvas::draw(QPainter & p) {
  ...
  const QPixmap * px =  ProfiledStereotypes::diagramPixmap(data->get_stereotype(),
                                                           the_canvas()->zoom());
  FILE * fp = svg();
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((PackageCanvas *) (*it))->name_in_tab = name_in_tab;
        }
        if (!st[1].name.isEmpty()) {
          ((PackageCanvas *) (*it))->show_context_mode = show_context_mode;
        }
        if (!st[2].name.isEmpty()) {
          ((PackageCanvas *) (*it))->show_stereotype_properties = show_stereotype_properties;
        }
        if (!co[0].name.isEmpty()) {
          ((PackageCanvas *) (*it))->itscolor = itscolor;
        }
        ((PackageCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo())
      break;
...
void PackageCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  PackageCanvas * x = (PackageCanvas *) (*it);

  it++;
  for (it = l.begin(); it!= l.end(); it++) {
    PackageCanvas * o =  (PackageCanvas *) (*it);
