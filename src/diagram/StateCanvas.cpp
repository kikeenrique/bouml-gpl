    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((StateCanvas *) (*it))->settings.show_activities = show_activities;
        }
        if (!st[1].name.isEmpty()) {
          ((StateCanvas *) (*it))->settings.region_horizontally = region_horizontally;
        }
        if (!st[2].name.isEmpty()) {
          ((StateCanvas *) (*it))->settings.drawing_language = language;
        }
        if (!co[0].name.isEmpty()) {
          ((StateCanvas *) (*it))->itscolor = itscolor;
        }
        ((StateCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void StateCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  StateCanvas * x = (StateCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    StateCanvas * o =  (StateCanvas *) (*it);
