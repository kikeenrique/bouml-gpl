  ...
  for (;;) {
    ...
    if ((dialog.exec() == QDialog::Accepted) && !co[0].name.isEmpty()) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        ((ExpansionRegionCanvas *) (*it))->itscolor = itscolor;
        ((ExpansionRegionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void ExpansionRegionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  ExpansionRegionCanvas * x = (ExpansionRegionCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    ExpansionRegionCanvas * o =  (ExpansionRegionCanvas *) (*it);
