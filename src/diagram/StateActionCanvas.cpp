    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!st[0].name.isEmpty()) {
          ((StateActionCanvas *) (*it))->language = language;
        }
        if (!st[1].name.isEmpty()) {
          ((StateActionCanvas *) (*it))->show_stereotype_properties = show_stereotype_properties;
        }
        if (!co[0].name.isEmpty()) {
          ((StateActionCanvas *) (*it))->itscolor = itscolor;
        }
        ((StateActionCanvas *) (*it))->modified();	// call package_modified()
      }
    }
    if (!dialog.redo()) {
      break;
...
void StateActionCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  StateActionCanvas * x = (StateActionCanvas *) (*it);
  it++;
  for (; it!= l.end(); it++) {
    StateActionCanvas * o =  (StateActionCanvas *) (*it);
