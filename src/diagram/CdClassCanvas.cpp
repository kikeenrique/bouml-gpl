...
void CdClassCanvas::check_inner() {
  QList<ArrowCanvas *>::iterator it;
  for (it = lines.begin(); it!= lines.end(); it++) {
    if ((*it)->type() == UmlInner) {
      DiagramItem * b = ((ArrowCanvas *) (*it))->get_start();
      DiagramItem * e = ((ArrowCanvas *) (*it))->get_end();
...
bool CdClassCanvas::has_relation(BasicData * def) const {
  QList<ArrowCanvas *>::const_iterator it;
  for (it = lines.begin(); it!= lines.end(); it++) {
    if (IsaRelation((*it)->type()) &&
        (((RelationCanvas *) (*it))->get_data() == def)) {
      return TRUE;
    }
  }
...
void CdClassCanvas::menu(const QPoint&) {
  QList<BrowserOperation *> l = ((BrowserClass *) browser_node)->inherited_operations(21);
  Q3PopupMenu m(0);
  ...
  if (!strcmp(stereotype, "enum")) {
    ...
    if (browser_node->is_writable() &&
      ...
      if (l.count() > 20) {
        ...
        for (it = l.begin(), index = 3000; it!= l.end(); it++, index += 1) {
  ...
  m.insertSeparator();
  if (browser_node->is_writable()) {
    if (browser_node->get_associated() != (BrowserNode *) the_canvas()->browser_diagram()) {
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (it = l.begin(); it!= l.end(); it++) {
        if (!co[0].name.isEmpty()) {
          ((CdClassCanvas *) (*it))->itscolor = itscolor;
        }
        ((CdClassCanvas *) (*it))->settings.set(st, 0);
        ((CdClassCanvas *) (*it))->modified();
        ((CdClassCanvas *) (*it))->package_modified();
      }
    }
    if (!dialog.redo()) {
      break;
...
void CdClassCanvas::same_drawing_settings(QList<DiagramItem *> & l) {
  ...
  CdClassCanvas * x = (CdClassCanvas *) (*it);
  for (; it!= l.end(); it++) {
    CdClassCanvas * o =  (CdClassCanvas *) (*it);
			     ...
			     const Q3ValueList<BrowserNode *> & hidden_visible)
{
  ...
  for (it = l.begin(); it!= l.end(); it++) {
    ...
    if (hidden_visible.findIndex((*it)) != -1) {
      if (s != 0) {
        nl_indent(st);
        st << s;
        s = 0;
