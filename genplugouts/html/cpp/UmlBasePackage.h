...
class UmlBasePackage : public UmlItem {
  public:
    ...
    // 0/null on error
    UmlPackage * importProject(const QByteArray & fn);
    ...
    static UmlPackage * findCppNamespace(const QByteArray & n, const UmlBasePackage * p = 0);
#endif
    ...
    // (including 'p' itself), else 0
    static UmlPackage * findJavaPackage(const QByteArray & n, const UmlBasePackage * p = 0);
#endif

#ifdef WITHPHP
    // returns the path where the PHP files are generated by the
    // PHP generator.
    const QByteArray & phpDir();
    ...
    // 
    // On error return FALSE in C++, produce a RuntimeException in Java
    bool set_PythonDir(const QByteArray & s);

    //returns the python package name associed to the package
    QByteArray pythonPackage();

    // to set the python package name associed to the package.
    // 
    // On error return FALSE in C++, produce a RuntimeException in Java
    bool set_PythonPackage(const QByteArray & s);

    // returns the sub package of 'p' package having the python package 'n'
    // (including 'p' itself), else 0
    static UmlPackage * findPythonPackage(const QByteArray & n, const UmlBasePackage * p = 0);
#endif
    ...
    // (including 'p' itself), else 0
    static UmlPackage * findIdlModule(const QByteArray & n, const UmlBasePackage * p = 0);
#endif
