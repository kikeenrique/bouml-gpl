...
void UmlClass::generalizeDependRealize(UmlItem * target, FileIn & in, int context, QByteArray label, QByteArray constraint) {
  static const struct {
    aRelationKind rk;
    const char * err;
  } r[] = {
...
void UmlClass::solveGeneralizationDependencyRealization(int context, QByteArray idref, QByteArray label, QByteArray constraint) {
...
UmlClass * UmlClass::signature(QByteArray id)
{
...
int UmlClass::formalRank(QByteArray id) {
...
void UmlClass::extend(QByteArray mcl) {
...
bool UmlClass::isAppliedStereotype(Token & tk, QByteArray & prof_st, QValueList<QByteArray> & base_v)
{
  static QDict<QByteArray> stereotypes;
  static QDict<QValueList<QByteArray> > bases;
  ...
  if (tk.xmiType().isEmpty() && (getFct(tk) == 0))  {
	...
	bases.insert(s, new QValueList<QByteArray>(base_v));
	return TRUE;
      }
    }
  }
...
UmlClass * UmlClass::addMetaclass(QByteArray mclname, const char * mclpath) {
...
QMap<QByteArray, UmlClass *> UmlClass::signatures;
